...
namespace MyClass8
{
    internal class Class1
    {
        ...
        public Class1(int number)     //숨어 있는 생성자
        {
            Console.WriteLine("number받는 생성자 {0} 이것이 바로 숨어 있는 생성자??", number);
        }
        ...
        public  void valueTypeAndRefferenceType()
        {
             ...
             *  전형적인 값 형식의 데이터 구조이다. // 통체로 가져와서 복사하는 것 // 원본을 수정한것은 반영되지 않는다.
             *  
             *  참조 형식
             *  개체가 값을 담고 있는 또 다른 개체를 포인터로 바라보는 구조이다. 여러 값이 동일한 개체를 
             *  가리킬 수 있다. // 주소만 가져와서 사용하는것 // 원본을 수정한 것이 반영이된다.
             *  
             */
             ...
             * 데이터로 변환하는 작럽을 의미한다. 예를 들어 다음 코드처럼 정수 형식의 데이터를 오브젝트 형식의
             * 데이터에 담는 형태를 박싱이라고 한다.
             * 
             *  int number = 1234;
             *  object objectValue = number;
             *  
             *  좀 더 어렵게 말하면 스택 메모리 영역에 저장된 값 형식의 데이터를 힘 메모리 영역에 저장하는
             *  단계를 거치기 때문에 시간과 공간이 소배되는 비용이 발생한다.
             *  
             * 언박싱
            ...
            object objectValue = 1234;
            char charValue = 'a';
            int number = (int)objectValue;      // int 변수에 저장 할려면 (int)로 변환하고 저장
            char charValue2;
        ...
        public void whatIsfield()
        {
             ...
             * 데이터를 담는 그릇 역할을 하낟. 이러한 필드는 개체 상태를 보관한다.
             * 필드는 선언한 후 초기화하지 않아도 자동으로 초기화한다. 예를 들어 int 형 필드는 0으로,
             * string 형 필드는 string.Empty, 즉 공백으로 초기화된다.
             * 
             * 값을 사용하기 전에 초기화를 하고 써라.
             * 
             * 지역 변수와 전역 변수
             * C#에서 변수를 건언할 떄는 Main() 메서드와 같은 메서드 내에서 선언하거나 메서드 밖에서,
             * 즉 메서드와 동등한 레벨이서 변수를 선언할 수 있다. 메서드 내에서 선언한 변수 또는 배열을 
        ...
        public void whatIsConstructor()
        {
             ...
             * 
             * 생성자
             * 클래스의 구성 요소 중에는 생성자(Constructor)라는 메서드가 숨어있다. 단어 그대로 개체를
             * 생성하면서 무엇인가를 하고자 할 때 사용되는 메서드이다. 일반적으로 생성자는 개체를 초기화
             * (주로 클래스 내 필드는 초기화)하는 데 사용된다. 생성자는 생성자 메서드라고도 한다. 이러한
             * 생성자는 독특한 규칙이 있는데, 바로 생성자 이름이 클래스 이름과 동일하다는 것이다. 클래스 내에서
             * 클래스 이름과 동일한 이름을 갖는 매서드는 모두 생성자이다.
             * 
             * 생성자는 매개변수가 없는 개본(Default) 생성자가 있고, 매개변수를 원하는 만큼 정의해서 사용할 
        ...
        public void whatIsDestructor()
        {
             ...
             * 메서드 이다. 자동차 클래스를 예를 들면 자동차 추자 대행, 시동 끄기 등으로 볼 수 있다.
             * 
             * 종료자      // 클래스를 알아서 닫아준다.
             ...
             *  - 소멸자도 특별한 형태의 메서드이다. 소멸자 또한 소멸자 메서드라고도 한다. 생성자와
             *  달리 매개변수를 받을 수 없다.
             *  - 소멸자는 오버로딩을 지원하지 않으면 직접 호출할 수도 없다.
             */
        }       // whatIsDestructor()
        ...
        public void WhatIsInheritance()
        {
             ...
             * 개체 관계 프로그래밍(Object relationsjip programming)이라고도 한다. 상속은 부모 클래스에
             * 정의된 기능을 다시 사용하거나 확장 수정하여 자식 클래스로 만드는 것이다. 특정 클래스에서
             * 만든 기능을 다른 크랠스에 상속으로 물려주면 장점이 많이 있다.
             * 
             * 클래스 상속하기
             * 개체 지향 프로그래밍의 장점 중 하나는 이미 만든 클래스를 재사용하는 것이다. 이 재사용의 핵심
             * 개념이 바로 상속이다. 부모 재산을 자식에게 상속하듯이 부모 클래스(기본 클래스)의 모든 맴버를
             ...
             * 
             * 클래스 상속 구분
             * C#에서는 다움과 같이 클래스 이름 뒤에 콜론(;) 기호와 부모가 되는 클래스 이름을 붙인다.
             * 
             ...
             *  자동으로 Object 클래스에서 상소받기에 Object 클래스를 상속하는 코드는 생략 가능하다.
             *  
             *  - 기본(Base) 클래스: 다른 클래스의 부모 클래스가 되는 클래스로 기본 클래스라고 한다.
             *    기본 클래스를 다르 말로 Base 클래스, Super 클래스, 부모 클래스로 표현한다.
             *  
             *  - 파생(Derived) 클래스: 다른 클래스의 자식 클래스가 되는 클래스를 파생 클래스라고 한다.
             *  파생 클래스는 다른 크래스에서 맴버를 물려받는 것으로, Dereved 클래스, sub 클래스,
             *  자식 클래스로 표현한다.
             *  
             * 부모 클래스와 자식 클래스
             * 프로그램밍에서 상속을 표현할 떄 상속을 주는 클래스를 부모 클래스라고 하며, 상속을 받는 클래스를
             * 자식 클래스라고 하낟. 콜론(:) 기호로 상속 관계를 지엉하면 부모 클래스의 public 맴버들을
    ...
    //        Console.WriteLine("부모 클래스의 멤버 변수 호출: {0} {1}", base.stringValue, base.intValue);  //습관에 따라서 base.을 쓰기도 하고 안쓰기도 하고
    //    }
    //}
